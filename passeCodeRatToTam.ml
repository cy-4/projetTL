...
let rec utiliser_id_pointeur affectable droite_instruction =
  ...
  | AstType.Ident(info) -> 
    ...
    |InfoVar (_,typ,base,r) ->
      if droite_instruction then
...
let analyse_code_affectable aff = 
  ...
  | AstType.Dref(affectable)-> 
    utiliser_id_pointeur affectable true
...
let rec analyse_code_instruction i (nb_p, t)  =
  ...
  | AstType.AffectationPointeur (aff,e) ->
    ...
    |Dref(affectable) -> 
      ((analyse_code_expression e)^(utiliser_id_pointeur affectable false),0)
  ...
  | AstType.Conditionnelle (c,f,e) -> 
    let vrai = getEtiquette () in
  ...
  | AstType.Retour (e) -> 
    ( analyse_code_expression e^ "RETURN ("^string_of_int t ^") "^string_of_int nb_p^"\n",0)
  | AstType.Empty-> ("",0) 
  end
...
and analyse_code_bloc li (nb_p, t) =
  ...
  let listeanalyse = (List.map (fun i -> analyse_code_instruction i (nb_p, t)) li) in 
...
let recuper_nombre_memoire listeparametre=
  ...
  | t::_ -> 
    ...
    | InfoVar(_,_,placement_derniere_variable,_) -> 
      -placement_derniere_variable
