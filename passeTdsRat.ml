...
match lp with
  | []  -> true
  | (_,nom)::q -> if (List.exists (function (_,nomverif) -> if nom=nomverif then (raise (DoubleDeclaration nom))  else false) q) then false 
        ...
        | Some info -> 
          ...
          match info_ast_to_info info with
            | InfoVar(_,typ,_,_) ->  
              ...
              | Type.TypeNomme(nom) -> convertirtypenomme tds (Type.TypeNomme(nom))
              (* On a bien un type non TypeNomme*)
              | _ -> typ
              end
